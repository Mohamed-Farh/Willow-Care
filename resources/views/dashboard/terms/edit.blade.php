        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card card-custom gutter-b example example-compact">
                    ...
                    <!--begin::Form-->
                    <form class="form" action="{{route('terms.update',$term->id)}}" method="POST" >
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                                ...
                                <div class=" col-sm-12">
                                    <select class="form-control" id="kt_select2_1" name="category">
                                        @foreach($category as $cat)
                                            <option {{ old('category',$term->category->id) == $cat->id ? "selected" : "" }} value="{{$cat->id}}">{{$cat->category}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group mb-0">
                                ...
                                <div class="checkbox-list">
                                    <label class="checkbox checkbox-outline">
                                        <input @if(old('active',$term->active)) checked @endif name="active" type="checkbox"/>
                                        <span></span></label>
                                </div>
                            </div>
