                    ...
                    <div class="card-header">
                        ...
                        <div class="card-toolbar">
                            <div class="example-tools justify-content-center">
                                <a href="{{route('insurance-company.index')}}" class="btn btn-warning mr-2">Back</a>
                            </div>
                        </div>
                    </div>
                    <!--begin::Form-->
                    <form class="form" action="{{route('insurance-company.update',$insuranceCompany->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            <div class="form-group">
                                <label>Arabic Name:</label>
                                <input type="text" class="form-control" name="name_ar" value="{{old('name_ar',$insuranceCompany->name_ar)}}"/>
                                @error('name_ar')<span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group">
                                <label>English Name:</label>
                                <input type="text" class="form-control" name="name_en" value="{{old('name_en',$insuranceCompany->name_en)}}"/>
                                @error('name_en')<span class="text-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="form-group">
                                <label>Roman Name:</label>
                                <input type="text" class="form-control" name="name_ro" value="{{old('name_ro',$insuranceCompany->name_ro)}}"/>
                            ...
                            <div class="form-group mb-0">
                                ...
                                <div class="checkbox-list">
                                    <label class="checkbox checkbox-outline">
                                        <input @if($insuranceCompany->active==1) checked @endif name="active" type="checkbox"/>
                                        <span></span></label>
                                </div>
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-9 col-xl-6">
                                        <div  class="image-input image-input-outline" id="kt_image_4"
                                              style="background-image: url({{asset($insuranceCompany->image)}})">
...
@section('scripts')
    ...
    <script>
        ...
        avatar4.on('remove', function(imageInput) {
            ...
            if(confirmed){
                $.ajax({
                    url: "{{ route('delCompanyImg', ['id'=>$insuranceCompany->id]) }}",
                    type: "POST",
